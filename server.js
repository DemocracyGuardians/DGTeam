...
var context = {
	optimize: true,
  mysqlconnection: null // set by mysq!Lconnect()
};

var mysqlPromise = mysqlconnect()
  .then( con => { context.mysqlconnection = con })
  .catch( err => { throw err; });
var requirejsBuiltPromise = new Promise(function(resolve, reject) { resolve(); });
var requirejsUncompressedPromise = new Promise(function(resolve, reject) { resolve(); });
var adminAccessPromise = new Promise(function(resolve, reject) { resolve(); });

var initPromises = Promise.all([ mysqlPromise, requirejsBuiltPromise, requirejsUncompressedPromise, adminAccessPromise ]);
...
var mysqlPromise = new Promise();
var requirejsBuiltPromise = new Promise();
var requirejsUncompressedPromise = new Promise();
var adminAccessPromise = new Promise();
var initPromises = [ mysqlPromise, requirejsBuiltPromise, requirejsUncompressedPromise, adminAccessPromise ];
*/
/*
...
// AllOrNone(context.initPromises).then(function(results) {
initPromises.then(function() {
...
/*
	// All of the http request listeners are defined in ./routes.js
	routes(app, context);	// Start web server and socket.io
	var options;
  ...
  */
  ...
  app.get('/team', function (req, res) {
    ...
    var port = ( req.headers.host.match(/:/g) ) ? req.headers.host.substr( req.headers.host.indexOf(":")+1 ) : 80;
...
app.get('/team', function (req, res) {
  ...
  var fullUrl = req.protocol + '://' + req.get('Host') + req.originalUrl;
  console.log((new Date()).toJSON(), 'GET /team entered. ', req.method, fullUrl, port);
  port === 80 ? res.send('Hello World!') : res.send(401);
});
